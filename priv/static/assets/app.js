...
`, template.innerHTML.trim());
  ...
  // js/hooks/info_text_animation.js
  var infoTextAnimation = (guessSubmitAnimation2) => ({
    mounted() {
      this.handleEvent("show-text-box", (data) => {
        validation = data.validation;
        const textBox = document.createElement("div");
        textBox.textContent = window.validationMessageKey[validation];
        textBox.classList.add("py-3", "px-3", "rounded-md", "absolute", "transition-opacity", "duration-500", "opacity-0", "bg-secondary-content", "text-base-100", "font-bold", "text-sm", "mx-auto", "-translate-y-7", "animate-none");
        textBox.id = "info_text_box";
        const topRow = document.getElementById("row_0");
        if (data.validation) {
          topRow.appendChild(textBox);
        }
        guessSubmitAnimation2(data.row, data.validation, this, data.answer);
        setTimeout(() => {
  ...
  var info_text_animation_default = infoTextAnimation;
  ...
  function guessSubmitAnimation(row, validation2, relay, answer) {
    const currentRow = document.getElementById(`row_${row}`);
    if (validation2 == "correct" || !validation2) {
      ...
      Array.from(childElements).forEach((childElement, index) => {
        setTimeout(() => {
          ...
          function permanentBackground(parentElement, answer2, index2) {
            const inputCell = parentElement.firstElementChild;
            inputCell.classList.add("border-none");
            if (inputCell.value.toLowerCase() == answer2[index2]) {
              return "bg-correct-index";
            } else if (answer2.includes(inputCell.value.toLowerCase())) {
          ...
          setTimeout(() => {
            childElement.classList.add(permanentBackground(childElement, answer, index));
          }, delay);
        }, index * 350);
      });
    } else {
  ...
  var Hooks2 = {
    infoTextAnimation: info_text_animation_default(guessSubmitAnimation),
